...
4. 유의 사항
AI 또는 시스템 생성 문구는 참고 자료이며 공식 접수 처리 결과가 아닙니다.
"""
...
def improve_draft_with_llm(draft_text: str) -> dict[str, Any]:
    prompt = (
        "다음 공문 초안을 공공기관 검토용 문체로 다듬어라. "
        "공식 접수 처리 결과로 오해될 표현은 쓰지 말고, 관리자 검증용 초안임을 유지한다.\n\n"
        f"{draft_text}"
    )
    ...
    if result.get("ok"):
        ...
        return result
    return {"ok": False, "source": result.get("source", "fallback"), "text": sanitize_public_claims(draft_text), "reason": result.get("reason", "")}
...
def can_send_email() -> dict[str, Any]:
    ...
    return {
        ...
        "data_status": "configured" if enabled and has_key and has_sender else "disabled_or_missing",
    }
    ...
    return {
        **status,
        "message": sanitize_public_claims(
            "SendGrid 실제 발송은 ENABLE_SENDGRID_SEND 설정, API 키, 발신 이메일, 사용자 확인 체크가 모두 필요합니다."
        ),
    }
...
def send_email_with_sendgrid(to_email: str, subject: str, body: str) -> EmailResult:
    ...
    if not status["can_send"]:
        return EmailResult(ok=False, message=sanitize_public_claims("SendGrid 발송 조건이 충족되지 않아 발송하지 않았습니다."), data_status="blocked")

    if not to_email:
        return EmailResult(ok=False, message=sanitize_public_claims("수신 이메일이 없어 발송하지 않았습니다."), data_status="missing_recipient")
    ...
    try:
        ...
        return EmailResult(ok=True, message=sanitize_public_claims("SendGrid 발송 요청이 처리되었습니다."), data_status="sent")
    except Exception as exc:
        return EmailResult(ok=False, message=sanitize_public_claims(f"SendGrid 발송 실패: {exc}"), data_status="send_error")
