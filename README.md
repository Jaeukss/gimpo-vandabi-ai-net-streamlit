...
- 이용자 경로분석: 출발지와 목적지 기준 참고 지도, 접근성 점수, 확인 필요 요인을 표시합니다.
...
2. `01 경로분석`에서 출발지와 목적지를 입력한 뒤 지도와 접근성 점수를 확인합니다.
...
- DATA_GO_KR_SERVICE_KEY: 8단계에서는 설정 상태만 확인하며 endpoint별 실제 호출은 9단계 대상입니다.
- SendGrid: 사용자가 확인하고 설정이 모두 충족된 경우에만 이메일 전송을 시도합니다.

## 8단계 배포 QA 체크리스트

- OpenRouter 텍스트 응답 또는 fallback 표시를 확인합니다.
- Vision 이미지 업로드 결과 또는 demo fallback 표시를 확인합니다.
- VWorld 주소 좌표 변환 또는 mock fallback 표시를 확인합니다.
- `ENABLE_SENDGRID_SEND = "false"` 상태에서 실제 이메일 전송이 차단되는지 확인합니다.
- B2G 대시보드에서 Secrets 상태가 configured/missing/disabled 수준으로만 표시되고 실제 키 값은 표시되지 않는지 확인합니다.
- DATA_GO_KR_SERVICE_KEY는 설정 상태만 확인하며 공공데이터 endpoint별 실제 호출은 9단계에서 진행합니다.
